...
def training_graph_ideal(G,p,is_sampled=True,r=1):
    edges = list(G.edges())
    num_edges = len(edges)
    num_test_edges = round((1-p)*num_edges)
...
def training_graph_not_ideal(G, train_pos_edges,train_non_edges,test_pos_edges,test_non_edges, overlap):
    max_replacement = min(overlap, len(test_pos_edges))
...
if __name__=='__main__':
    ...
    for i,data in enumerate(dataset):
        ...
        for j in range(n_loops):
            ...
            overlap_number = calculate_overlap_expectation(G, G2, p, r=1)
            print(overlap_number)
            set_random_seed(random_seed)
            auc_ideal[i,j] = extra_and_pre(G2, train_pos_edges,train_non_edges,test_pos_edges,test_non_edges)
            set_random_seed(random_seed)
            G2_ni, train_pos_edges_ni, train_non_edges_ni, test_pos_edges_ni, test_non_edges_ni = training_graph_not_ideal(G2, train_pos_edges,train_non_edges,test_pos_edges,test_non_edges,overlap=overlap_number)
            set_random_seed(random_seed)
